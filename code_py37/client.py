...
default_port = 54123
lan_ip, ext_ip, nx_nic = code_py37.engine.get_public_private_ip(verbose=True)
private_key = code_py37.engine.load_private_key(ext_ip.replace('.','')+'.pem')
...
def get_file(remote_host, query):
    ...
    if not os.path.isfile(rmt_key):
        print('[!!] No Public Key for %s. Run python client.py add %s' % (remote_host,
                                                                          remote_host))
        exit()
    rmt_pub_key = code_py37.engine.load_private_key(rmt_key)
    ...
    if os.path.isfile(query):
        if input('[!!] %s Already Exists, do you want to Overwrite it (y/n)?: ' % query).upper() == 'Y':
            os.remove(query)
...
def put_file(remote_host, file_name):
    ...
    encrypted_data = code_py37.utils.EncodeAES(AES.new(key), raw_file_data)
...
def query(remote_host, remote_key_file, cmd):
    if not os.path.isfile(remote_key_file):
        ...
        exit()
    # Load Key
    rmt_pub_key = code_py37.engine.load_private_key(remote_host.replace('.', '') + '.pem')
    ...
    decrypted_data = code_py37.utils.DecodeAES(AES.new(key), reply.split('::::')[1])
    print('[*] Reply:\n$ %s' % decrypted_data)
...
if __name__ == '__main__':
    ...
    if 'query' in sys.argv and len(sys.argv) >= 4:
        rmt = sys.argv[2]
        r_key = rmt.replace('.', '') + '.pem'
        q = code_py37.utils.arr2str(sys.argv[3:])
