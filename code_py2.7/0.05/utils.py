...
def encrypt_file_aes(content, file_name_out):
...
def decrypt_file_aes(file_name, file_out, destroy):
...
def create_rsa_key(name):
    ...
    pbk = key.publickey()
    open('%s_private' % name, 'wb').write(key.exportKey('PEM'))
    open('%s_public' % name,'wb').write(pbk.exportKey())
    return key
...
def encrypt_file_rsa(content, keyfile):
    if os.path.isfile(keyfile):
        key = RSA.importKey(open(keyfile, 'rb').read())
        pbk = key.publickey(())
    else:
        print '[!!] Unable to Find KeyFile: %s' % keyfile
        exit()
    rsa_cipher = PKCS1_OAEP.new(key)
    session_key = get_random_bytes(16)
    # Don't actually encrypt a message  with RSA, but the key for AES that is used
    enc_session_key = rsa_cipher.encrypt(session_key)
    cipher = AES.new(session_key)
    encrypted_data = EncodeAES(cipher, content)
    return enc_session_key, encrypted_data
